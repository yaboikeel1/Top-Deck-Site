...
$("generateTasksBtn").onclick=generateSuggestedTasks;
...
async function loadCRM(){
  ...
  const [artistsResult,releasesResult,tasksResult,notificationsResult,healthResult]=await Promise.all([
    ...
    client.from("release_health").select("*").order("last_checked",{ascending:false})
  ]);
  ...
  operationsTasks=tasksResult.error?[]:(tasksResult.data||[]);
  operationsNotifications=notificationsResult.error?[]:(notificationsResult.data||[]);
  operationsHealth=healthResult.error?[]:(healthResult.data||[]);
...
function healthForRelease(release){
  const data=readinessData(release);
  const days=release.release_date?Math.ceil((new Date(release.release_date)-new Date())/86400000):null;
  let score=data.score;
  const reasons=[];
  if(!data.values.audio){score=Math.min(score,35);reasons.push("Audio missing");}
  if(!data.values.artwork){score=Math.min(score,45);reasons.push("Artwork missing");}
  if(days!==null&&days>=0&&days<=7&&data.score<100){score=Math.max(0,score-12);reasons.push(`Release is ${days} day${days===1?"":"s"} away`);}
  if(!data.values.marketing){reasons.push("Marketing assets incomplete");}
  let status="Healthy";
  if(score<50)status="Critical"; else if(score<85)status="Needs Attention";
  const recommendation=reasons[0]||data.missing[0]||"Release is ready to move forward";
  return {score:Math.max(0,Math.min(100,score)),status,recommendation,data};
}

function renderOperationsPanels(){
  const openTasks=operationsTasks.filter(t=>!["Complete","Completed","Done"].includes(t.status||"Open"));
  $("taskPanelCount").textContent=openTasks.length;
  $("liveTasks").innerHTML=openTasks.length?openTasks.slice(0,7).map(task=>{
    const release=releases.find(r=>String(r.id)===String(task.release_id));
    return `<div class="task-row"><div class="task-row-top"><div><strong>${escapeHtml(task.title||"Untitled task")}</strong><div class="ops-meta"><span class="priority-${escapeHtml((task.priority||"Medium").toLowerCase())}">${escapeHtml(task.priority||"Medium")}</span> • ${escapeHtml(task.assigned_to||"Unassigned")}${task.due_date?` • Due ${escapeHtml(formatDate(task.due_date))}`:""}</div></div><button class="micro-button" onclick="completeTask('${task.id}')">Complete</button></div>${release?`<div class="ops-meta">${escapeHtml(release.topdeck_release_id||"ID pending")} • ${escapeHtml(release.title||"Untitled Release")}</div>`:""}</div>`;
  }).join(""):'<div class="empty">No open tasks. Generate suggestions to create a working list.</div>';

  const unread=operationsNotifications.filter(n=>n.is_read!==true);
  $("notificationPanelCount").textContent=unread.length;
  $("liveNotifications").innerHTML=unread.length?unread.slice(0,7).map(note=>`<div class="notification-row"><div class="notification-row-top"><div><strong>${escapeHtml(note.title||"Notification")}</strong><div class="ops-meta">${escapeHtml(note.message||"")} • ${escapeHtml(formatDate(note.created_at))}</div></div><button class="micro-button" onclick="markNotificationRead('${note.id}')">Read</button></div></div>`).join(""):'<div class="empty">You are all caught up.</div>';

  const healthRows=releases.map(release=>({release,...healthForRelease(release)})).sort((a,b)=>a.score-b.score);
  $("healthPanelCount").textContent=healthRows.filter(x=>x.status!=="Healthy").length;
  $("liveHealth").innerHTML=healthRows.length?healthRows.slice(0,7).map(item=>{
    const artist=artistForRelease(item.release);
    const cls=item.status==="Healthy"?"health-healthy":item.status==="Critical"?"health-critical":"health-attention";
    return `<div class="health-row"><div class="health-row-top"><div><strong><span class="health-dot ${cls}"></span>${escapeHtml(item.release.title||"Untitled Release")} — ${item.score}%</strong><div class="ops-meta">${escapeHtml(artist?.artist_name||"Unknown Artist")} • ${escapeHtml(item.status)} • ${escapeHtml(item.recommendation)}</div></div><button class="micro-button" onclick="openReleaseWorkspace('${item.release.id}')">Open</button></div></div>`;
  }).join(""):'<div class="empty">No releases available.</div>';
}

async function generateSuggestedTasks(){
  const suggestions=[];
  releases.forEach(release=>{
    const info=priorityForRelease(release);
    if(info.data.score===100)return;
    suggestions.push({title:info.action,description:`Auto-generated from release readiness (${info.data.score}%).`,assigned_to:info.owner,priority:info.data.score<50?"High":info.data.score<80?"Medium":"Low",status:"Open",release_id:release.id,artist_id:release.user_id||null,due_date:release.release_date||null});
  });
  if(!suggestions.length)return setNotice("commandStatus","No suggested tasks are needed. Every release is complete.","success");
  const existing=new Set(operationsTasks.filter(t=>!["Complete","Completed","Done"].includes(t.status||"Open")).map(t=>`${t.release_id}|${t.title}`));
  const payload=suggestions.filter(t=>!existing.has(`${t.release_id}|${t.title}`));
  if(!payload.length)return setNotice("commandStatus","Suggested tasks are already up to date.","success");
  setNotice("commandStatus","Generating suggested tasks...");
  const {error}=await client.from("tasks").insert(payload);
  if(error)return setNotice("commandStatus",error.message,"error");
  await client.from("activity_log").insert({action:"Suggested tasks generated",action_details:`${payload.length} task(s) created by the V7.4 Action Center.`,performed_by:"Top Deck OS"});
  setNotice("commandStatus",`${payload.length} suggested task${payload.length===1?"":"s"} created.`,"success");
  await loadCRM();
}

window.completeTask=async id=>{
  const {error}=await client.from("tasks").update({status:"Complete"}).eq("id",id);
  if(error)return setNotice("commandStatus",error.message,"error");
  await client.from("activity_log").insert({action:"Task completed",action_details:"An Action Center task was marked complete.",performed_by:"Top Deck Admin"});
  await loadCRM();
};
...
async function logReleaseActivity(release,action,details){
  const payload={release_id:release.id,artist_id:release.user_id||null,action,action_details:details,performed_by:"Top Deck Admin"};
...
async function saveReleaseWorkspace(){
  ...
  await logReleaseActivity(release,"Release workspace updated",`Readiness ${readiness.score}% • ${readiness.workflowStatus}`);
  if(readiness.score===100 && release.readiness_score!==100){
    await client.from("notifications").insert({title:"Release ready for distribution",message:`${release.title||"A release"} reached 100% readiness.`,notification_type:"success",release_id:release.id,artist_id:release.user_id||null});
  }
  workspaceDirty=false;
  renderSaveState();
